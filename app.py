...
@app.route("/criar_conta", methods=["GET"])
def criar_conta():
    return render_template("criar_conta.html")

@app.route("/recuperar_conta", methods=["GET"])
def recuperar_conta():
...
# Rota para atualizar uma nota específica de um item
@app.route("/lista/<int:lista_id>/item/<int:item_id>", methods=["PUT"])
def atualizar_nota(lista_id, item_id):
    anotacoes_item = request.form.get("anotacoes_item")

    conn = create_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE Itens SET Anotacoes_item = %s WHERE idItens = %s", (anotacoes_item, item_id))
    conn.commit()
...
def adicionar_item(lista_id):
    ...
    anotacoes_item = request.form.get("anotacoes_item", "")  # Anotação opcional
    conn = create_connection()
    cursor = conn.cursor()
    
    # Inserindo o novo item na tabela Itens
    cursor.execute("INSERT INTO Itens (Nome_Item, Anotacoes_item) VALUES (%s, %s)", (nome_item, anotacoes_item))
    item_id = cursor.lastrowid
    
    # Associando o item à lista
...
def obter_itens(lista_id):
    ...
    # Consultando os itens e suas anotações associados a uma lista específica
    query = """
